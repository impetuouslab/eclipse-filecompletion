...
public class MarkerFinderImpl extends CompilationParticipant {
	...
	public void reconcile(final ReconcileContext context) {
		...
		try {
			...
			// LOG.info("changed element "+ context.getDelta().getElement());
			final CompilationUnit compilationUnit = context.getDelta()
					.getCompilationUnitAST();
			ITypeRoot typeRoot = compilationUnit.getTypeRoot();
			resource = typeRoot.getResource();
			l: if (lastVerifiedResource != null
					&& lastVerifiedResource.equals(resource.toString())) {
				// avoiding too often invocations
				if (System.currentTimeMillis() - lastVerifiedTime < 5000) {
					return;
				}
			}
			...
			List<StringLiteral> foundeFilesNodes = fileClassFinderVerifier
					.getFoundedFileNodes();
			...
			for (StringLiteral stringLiteral : foundeFilesNodes) {
				...
				if (!fileExists) {
					createFileMarker(stringLiteral, file, compilationUnit,
							resource);
				}
			}

			// validating reg exp
			List<StringLiteral> foundedPatternNodes = fileClassFinderVerifier
					.getFoundedPatternNodes();
			for (StringLiteral stringLiteral : foundedPatternNodes) {
				...
				try {
					...
					createPatternMarker(stringLiteral, literalValue,
							compilationUnit, resource, e.getMessage());
				} finally {
					long duration = System.currentTimeMillis() - startTime;
					if (duration > 200) {
						StringBuilder stringBuilder = new StringBuilder();
			...
			lastVerifiedResource = resource.toString();
	...
	public void createFileMarker(StringLiteral stringLiteral, File file,
			CompilationUnit compilationUnit, IResource resource)
		...
		marker.setAttribute(IMarker.LINE_NUMBER,
				compilationUnit.getLineNumber(stringLiteral.getStartPosition()));
		marker.setAttribute(IMarker.CHAR_START,
				stringLiteral.getStartPosition() + 1);
		marker.setAttribute(IMarker.CHAR_END, stringLiteral.getStartPosition()
				+ stringLiteral.getLength() - 1);
	}

	public void createPatternMarker(StringLiteral stringLiteral,
			String patteren, CompilationUnit compilationUnit,
			IResource resource, String reasonText) throws CoreException {
		LOG.fine("adding marker to " + resource);
		IMarker marker = resource.createMarker(markerId);
		marker.setAttribute(IMarker.MESSAGE, "Pattern invalid: " + patteren
				+ " , " + reasonText);
		marker.setAttribute(IMarker.PRIORITY, IMarker.PRIORITY_NORMAL);
		marker.setAttribute(IMarker.SEVERITY, IMarker.SEVERITY_WARNING);
		marker.setAttribute(IMarker.LINE_NUMBER,
